...
export class UserService {
  ...
  authenticate(auth: AuthRequest) {
    ...
    firstValueFrom(this.postAuthRequest(auth))
      ...
      .catch((err) => {
        if (err.status == 401)
          console.error('UNAUTHORIZED: WRONG/USERNAME PASSWORD');
        if (err.status == 500)
  ...
  signup(signup: RegisterRequest): Observable<boolean> {
    ...
    return this.postRegisterRequest(signup).pipe(
      map((resp: string) => {
  ...
  updateUser(profileForm: FormGroup): Observable<any> {
    let newUpdate: UpdateRequest = {
      userID: this.userID as string,
      customerEmail: profileForm.get('customerEmail')!.value,
      customerPhone: profileForm.get('customerPhone')!.value,
      shippingAddress: profileForm.get('shippingAddress')!.value,
