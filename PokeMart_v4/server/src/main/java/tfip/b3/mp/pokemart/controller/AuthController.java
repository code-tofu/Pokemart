...
import tfip.b3.mp.pokemart.model.UpdateRequest;
...
public class AuthController {
    ...
    public ResponseEntity<String> registerCustomer(@RequestBody RegisterRequest registerRequest) {
        ...
        return ResponseEntity.status(HttpStatus.CREATED)
                .body((Json.createObjectBuilder().add("userID", newUser)).build().toString());
    }
    ...
    @PostMapping("/api/user/updateCustomer")
    public ResponseEntity<String> updateCustomer(@RequestBody UpdateRequest updateRequest) {
        int updated = userSvc.updateUser(updateRequest);
        System.out.println(">> [INFO] Updated: " + updated + " Entry");
        return ResponseEntity.status(HttpStatus.CREATED)
                .body((Json.createObjectBuilder().add("Profile Updated", updated + " Entry")).build().toString());
    }
